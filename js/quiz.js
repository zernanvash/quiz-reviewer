  ...
  checkAnswer(question) {
    ...
    switch (question.type) {
      case "multiple_response":
        // Check if user selected anything and if it matches the correct array
        if (!Array.isArray(question.userAnswer) || !question.correctAnswer)
          return false;
        if (question.userAnswer.length !== question.correctAnswer.length)
          return false;

        // Sort both to ensure order doesn't matter, then compare
        const userRes = [...question.userAnswer].sort();
        const correctRes = [...question.correctAnswer].sort();
        return correctRes.every((val, index) => val === userRes[index]);
